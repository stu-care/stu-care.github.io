...
import { currency, Rates, CoinDenominations } from "../../helpers/currency";
    ...
    useEffect(() => {
        const totalInGold = Object.entries(purse.values).reduce(
            (total, [abbr, amount]) => {
                return (
                    total +
                    currency.convert(amount, abbr as CoinDenominations, "gp")
                );
            },
            0
        );
        setTotal(
            currency.convert(
                totalInGold,
                "gp",
                totalCurrency as CoinDenominations
            )
        );
                ...
                {denominations.map((abbr, index) => (
                    <div key={abbr} className="flex flex-row items-center">
                        ...
                        {index > 0 && (
                            <button
                                onClick={() =>
                                    handleTransferFromHigherDenomination(abbr)
                                }
