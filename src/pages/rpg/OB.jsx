...
const OBPage = () => {
    ...
    const [movementRate, setMovementRate] = useState(
        faldrin?.bmr + faldrin?.stats.qu.total
    );
    const [movement, setMovement] = useState(0);
    const [totalOb, setTotalOb] = useState(
        faldrin?.weapons[0]?.bonuses.total ?? 0
    );
    const [remainingOb, setRemainingOb] = useState(null);
    ...
    useEffect(() => {
        ...
        breadcrumbs.add("/rpg/ob", obTitle);
    }, []);
        ...
        <main className="relative p-4 grid grid-flow-row auto-rows-fr h-full gap-4 font-[Grenze]">
            <div className="flex items-center justify-center h-full flex-col gap-2">
                ...
                <input
                    ...
                    value={movementRate}
                    onChange={(e) => {
                        setMovementRate(e.target.value);
                    }}
                    step={5}
